...
sys.path.append(o_path + "MultiTune/advisor/av_files/algorithms/")
...
res_savepath = o_path + "MultiTune/advisor/av_files/dqn/"
enc_rdc_model_path = o_path + "MultiTune/advisor/av_files/encoder_reducer/"
model_dir = o_path + "MultiTune/advisor/av_files/dqn/"
...
def set_budget_timer(budget):
    t = threading.Timer(budget, budget_run_out)
    global budget_timer
    budget_timer = t
    budget_timer.start()
...
def reset_budget_flag():
    global BUDGET_FLAG
    BUDGET_FLAG = False


def reset_budget_timer():
    if (not BUDGET_FLAG):
        global budget_timer
        budget_timer.cancel()
...
class Environment(object):
    ...
    def __init__(self,
                 budget=100000,
                 max_mv_num=MAX_MV_NUM,
                 max_q_mv_edge_num=MAX_Q_MV_EDGE_NUM,
                 use_real_time=False,
        ...
        self.encoder_reducer = Encoder_reducer(context_feature)
    ...
    def reset(self):
        self.budget_used = 0
        self.selected_mv_bitmap = [0] * (len(self.mv_data)
                                         if self.mv_data else self.max_mv_num)
        self.selected_q_mv_edge_bitmap = [0] * (len(
            self.q_mv_data) if self.q_mv_data else self.max_q_mv_edge_num)
    ...
    def step(self, action):
        ...
        if action == 0:
            ...
            if self.selected_mv_bitmap[
                    cur_mv_index] == 1 or view.size + self.budget_used <= self.budget:
        ...
        for qv_pair, selected in zip(self.estimated_pairs,
                                     self.selected_q_mv_edge_bitmap):
            if selected:
                query = qv_pair.query
                view = qv_pair.view
        ...
        for q_id in self.query_index.keys():
            ...
            if q_candi_q_mv[q_id]:
                new_tim = min([
                    qv_pair.query_view_execution_time
                    for qv_pair in q_candi_q_mv[q_id]
                ])
        ...
        # go to next edge
        self.current_edge_index = (self.current_edge_index + 1) % len(
    ...
    def load_data(self):
        ...
        self.q_mv_data = csv_data_loader(trainset_dir + "query-mvdata.csv",
                                         [3])
        ...
        for q_id, mv_id, q_mv_id in self.triples:
            evl_q_tim, evl_q_mv_tim, _, hidden = self.encoder_reducer.evaluate(
                self.q_data[q_id][0], self.mv_data[mv_id][0])
...
class DQN(object):
    ...
    def select_action(self, state, steps_done):
        threshold = self.EPS_END + (self.EPS_START - self.EPS_END) * math.exp(
    ...
    def optimize_model(self):
        ...
        next_state_exist_mask = torch.tensor(
        ...
        state_action_values = self.policy_net(batch_state).gather(
            1, batch_action)

        expect_next_state_values = torch.zeros((self.BATCH_SIZE),
                                               device=self.device)
        expect_next_state_values[next_state_exist_mask] = self.target_net(
        ...
        loss = F.smooth_l1_loss(state_action_values,
                                expect_next_state_values.unsqueeze(1))
    ...
    def train(self,
              num_episodes,
              preprocecssed_selected_mv_bitmap=None,
              preprocecssed_selected_q_mv_edge_bitmap=None,
              use_pre_DP=True):
        steps_done = 0
        self.epoch = 0
        ...
        if use_pre_DP:
            ...
            preprocecssed_selected_mv_bitmap = gen_mv_bitmap_from_mv_list(
                self.env, choosed_mv_list)
            preprocecssed_selected_q_mv_edge_bitmap = gen_edge_bitmap_from_mv_bitmap(
                self.env, preprocecssed_selected_mv_bitmap)
            self.env.reset()

        if len(self.max_opti_solution) == 0:
            self.max_opti_solution = self.env.reset()
            self.max_optimize_tim = 0

        for episodes_id in range(num_episodes):
            if (BUDGET_FLAG):
                with open(res_savepath + "log.txt", "a") as fd:
                    fd.write("budget used up while doing train()\n")
                break
            ...
            for turns in count():
                ...
                steps_done += 1
                env_next_state, reward, done, info = self.env.step(
                    action.item())
                ...
                if env_next_state["edge_index"] == 0:
                    self.episodes_optimize_times.append(
                        env_next_state["total_delta_time"])

                if done or turns > MAX_TURNS or BUDGET_FLAG:
                    print(tmp_max_opti_time)
                    # self.episodes_optimize_times.append(tmp_max_opti_time)
                    if use_tensorboardX:
                        writer.add_scalar('Train/optimize_time',
                                          tmp_max_opti_time, episodes_id)
    ...
    def env_state2tensor(self, e):
        ...
        lis = [
            e["edge_bitmap"][e["edge_index"]], e["mv_bitmap"][e["mv_index"]],
            e["edge_index"], e["mv_index"], e["budget_used"] / e_budget,
            e["mv_size"] / e_budget, e["q_mv_delta_time"]
        ]
    ...
    def plot_durations(self):
        ...
        plt.clf()
        opti_tim_t = torch.tensor(self.episodes_optimize_times,
                                  dtype=torch.float,
                                  device=self.device)
        ...
        plt.savefig(res_savepath +
                    "episode-opti_times-budget{0}.pdf".format(self.env.budget))
        with open(
                res_savepath +
                "episode-opti_times-budget{0}.json".format(self.env.budget),
                "w") as fp:
    ...
    def log_latency(self, start, stop, turns):
        with open(res_savepath + "log_latency.json".format(self.env.budget),
                  "w") as fp:
    ...
    def plot_durations2(self):
        ...
        plt.savefig(res_savepath +
                    "rounds-opti_times-budget{0}.pdf".format(self.env.budget))
        with open(
                res_savepath +
                "rounds-opti_times-budget{0}.json".format(self.env.budget),
                "w") as fp:
            json.dump(self.episodes_optimize_times, fp)
    ...
    def plot_solution(self, max_opti_solution=None):
        ...
        nx.draw_networkx_nodes(G,
                               pos=node_pos,
                               nodelist=choosed_mv_list,
                               node_color='r')
        nx.draw_networkx_edges(G,
                               pos=node_pos,
                               edgelist=choosed_edge_list,
                               edge_color='r',
                               width=3)
        ...
        plt.savefig(
            res_savepath +
            "solution-budget{0}.pdf".format(max_opti_solution["budget"]))

        # plt.show()

    def save_model(self, budget):
        torch.save(self.policy_net,
                   res_savepath + "policy_net{0}.pth".format(budget))

    def load_model(self, budget):
        self.policy_net = torch.load(model_dir +
                                     "policy_net{0}.pth".format(budget),
                                     map_location="cpu")
    ...
    def DP(self, budget=500, save_result=True):
        ...
        f = [[[0, 0] for j in range(budget + 1)]
             for i in range(len(mv_info_list))]
...
class Q(object):
    ...
    def __init__(self, arg_list):
        self.id = arg_list[0]
        self.seq = arg_list[1]
        self.cost = arg_list[2]
        self.execution_time = arg_list[3]
...
class V(object):
    ...
    def __init__(self, arg_list):
        ...
        self.seq = arg_list[1]
        self.cost = arg_list[2]
        self.time = arg_list[3]
        self.size = arg_list[4]
...
class QVpair(object):
    ...
    def __init__(self, arg_list):
        ...
        self.seq = arg_list[1]
        self.cost = arg_list[2]
        self.query_view_execution_time = arg_list[3]
        self.query = arg_list[4]
        self.view = arg_list[5]
        self.benefit = arg_list[6]
        self.embedding = arg_list[7]


def env_build(env):

    def csv_data_loader(filename, col_id_lis, type="d"):
        res_data = list()
        with open(filename) as fp:
            csv_reader = csv.reader(fp)
            if type == "q":
                ...
                for line in csv_reader:
                    ...
                    for i in col_id_lis:
                        if i == 1:
                            ...
                            tmp_lis.append(line[i])
                    # rescale size from B to MB
                    tmp_lis[4] = tmp_lis[4] / 1e6
                    res_data.append(V(tmp_lis))
            elif type == "d":
                for line in csv_reader:
                    tmp_lis = []
                    for i in col_id_lis:
                        if i == 1:
                            ...
                            tmp_lis.append(line[i])
                    res_data.append(tmp_lis)
        return res_data

    # read q: id, seq, cost, time
    workload = csv_data_loader(trainset_dir + "querydata.csv", [0, 1, 2, 3],
                               "q")

    # read mv: id, seq, cost, time, size
    views = csv_data_loader(trainset_dir + "mvdata.csv", [0, 1, 2, 3, 4], "v")

    # read q-mv: id, seq, cost, time
    q_mv_data = csv_data_loader(trainset_dir + "query-mvdata.csv",
                                [0, 1, 2, 3])

    # read q_mv_q-mv index
    with open(trainset_dir + "query_mv_q_mv_index.csv") as fp:
        csv_reader = csv.reader(fp)
        triples = list(csv_reader)

    estimated_pairs = list()
    for triple in triples:
        q = [x for x in workload if x.id == triple[0]][0]
        v = [x for x in views if x.id == triple[1]][0]
        q_mv = [x for x in q_mv_data if x[0] == triple[2]][0]
        evl_q_tim, evl_q_mv_tim, _, hidden = env.encoder_reducer.evaluate(
            q.seq, v.seq)
        if not env.use_real_time:
            q.execution_time = evl_q_tim
            q_mv[3] = evl_q_mv_tim
            benefit = q.execution_time - v.time - q_mv[3]
        else:
            benefit = q.execution_time - v.time - q_mv[3]
        q_mv.append(q)
        q_mv.append(v)
        q_mv.append(benefit)
        q_mv.append(hidden)
        estimated_pairs.append(QVpair(q_mv))

    env.build(workload, views, estimated_pairs)


def train_dqn_with_budget(time_budget=180, mv_budget=500, context_feature=None):
    set_budget_timer(time_budget)

    use_pre_DP = False
    env = Environment(budget=mv_budget, use_real_time=True, context_feature=context_feature)
    env.load_data()
    env_build(env)
    ...
    if use_pre_DP:
        choosed_mv_list = dqn.DP(mv_budget, True)
        selected_mv_bitmap = gen_mv_bitmap_from_mv_list(env, choosed_mv_list)
        selected_q_mv_edge_bitmap = gen_edge_bitmap_from_mv_bitmap(
            env, selected_mv_bitmap)
    else:
    ...
    dqn.train(
        time_budget,
        preprocecssed_selected_mv_bitmap=selected_mv_bitmap,
        preprocecssed_selected_q_mv_edge_bitmap=selected_q_mv_edge_bitmap)

    with open(res_savepath + "budget-max_opti.txt", "a") as fp:
        fp.write(str(mv_budget) + " " + str(dqn.max_optimize_tim) + "\n")
        fp.write(" ".join(map(str, dqn.max_opti_solution["mv_bitmap"])) + "\n")
        fp.write(" ".join(map(str, dqn.max_opti_solution["edge_bitmap"])) +
                 "\n")
        dqn.save_model(mv_budget)

    reset_budget_timer()
    reset_budget_flag()


def evaluate_with_budget(time_budget=180,
                         mv_budget=500,
                         max_opti_solution=None,
                         max_optimize_tim=-1,
                         context_feature=None):
    set_budget_timer(time_budget)

    if not os.path.exists("policy_net{0}.pth".format(mv_budget)):
        train_dqn_with_budget(time_budget=int(time_budget / 2),
                              mv_budget=mv_budget, context_feature=context_feature)
    ...
    dqn = DQN(env)
    dqn.load_model(mv_budget)

    if max_opti_solution is not None:
        dqn.max_opti_solution = max_opti_solution
        dqn.max_optimize_tim = max_optimize_tim
    ...
    with open(res_savepath + "evaluate_result.txt", "a") as fp:
        fp.write(str(mv_budget) + " " + str(dqn.max_optimize_tim) + "\n")
        fp.write(" ".join(map(str, dqn.max_opti_solution["mv_bitmap"])) + "\n")
        fp.write(" ".join(map(str, dqn.max_opti_solution["edge_bitmap"])) +
                 "\n")
    ...
    reset_budget_timer()
    reset_budget_flag()
    return dqn.max_optimize_tim, dqn.max_opti_solution
...
if __name__ == "__main__":
    ...
    env = Environment(budget=budget, use_real_time=False)
    env.load_data()
    ...
    # train
    print(
        "use GAMMA0.98, decay 1000, EPS end 0.1, LeakyReLU negative_slope=0.01, use_pre_DP"
    )
    print("begin")
    ...
    for budget in train_budget_list:
        ...
        if use_pre_DP:
            choosed_mv_list = dqn.DP(budget, True)
            selected_mv_bitmap = gen_mv_bitmap_from_mv_list(
                env, choosed_mv_list)
            selected_q_mv_edge_bitmap = gen_edge_bitmap_from_mv_bitmap(
                env, selected_mv_bitmap)
        else:
