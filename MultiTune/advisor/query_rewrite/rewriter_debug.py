...
class Rewriter():
    ...
    def single_rewrite(self, sql0, ruleid):
        ...
        format_sql = self.parse_quote(sql0.lower()).replace('`', '')
        # format_sql = sql
        # print(format_sql)
        ...
        if is_rewritten == 1:  # the rule can be applied
            ...
            sql1 = self.RA2SQL1(ra)
            sql2 = self.RA2SQL(ra)
            #if not sql1 == sql2:
            #    pdb.set_trace()
            #print("original query:{}\n".format(sql0))
            #print("rewriten query1:{}\n".format(sql1))
            #print("rewriten query2:{}\n".format(sql2))
            #print('---------------------------')

            success, res = self.db.validate_sql(str(sql2))
            if not success:
                pdb.set_trace()
                success, res = self.db.validate_sql(str(sql2))
            if success == 1:
